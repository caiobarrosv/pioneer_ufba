...
{  
  ...
  std::string modelName = (std::string)"p3dx" ;
  ...
  while (ros::ok())
  {
      // Requisita o estado do robo no gazebo
      ...
      // Obtem o estado do robo no gazebo
      ...
      // Obtem a velocidade linear e angular atual
      current_Twist = getModelState.response.twist ;
      
      // Seta a posicao do frame odom na mesma posicao do robo
      ...
      // Seta a orientacao do frame odom na mesma posicao do robo
      geometry_msgs::Quaternion geo_Quat ;
      tf::quaternionTFToMsg(tf_quat, geo_Quat) ;
      odom_trans.transform.rotation = geo_Quat ;

      // Seta o base_link como frame filho
