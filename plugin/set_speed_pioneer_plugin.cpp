...
namespace gazebo
{
  // Modele plugin é o base class - base class (parent) - the class being inherited from
  class ModelJointControler : public ModelPlugin
  {
    // Public é o Access specifier
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
      ...
      // valores setados no modelo sdf onde tem o plugin
      ...
      if(this->activate_pid_control)
      {
          ...
          jointController->AddJoint(model->GetJoint("base_right_wheel_joint"));
          jointController->AddJoint(model->GetJoint("base_left_wheel_joint"));

          this->right_wheel_name = model->GetJoint("base_right_wheel_joint")->GetScopedName();
          this->left_wheel_name = model->GetJoint("base_left_wheel_joint")->GetScopedName();
	  ...
	  jointController->SetVelocityTarget(this->right_wheel_name, this->right_wheel_speed_magn);
        ...
        }else
        {
            ...
            ROS_DEBUG("Update Wheel Speed BASIC...");
      	    this->model->GetJoint("base_right_wheel_joint")->SetVelocity(0, this->right_wheel_speed_magn);
            this->model->GetJoint("base_left_wheel_joint")->SetVelocity(0, this->left_wheel_speed_magn);
        }
    ...
    public: void SetLeftWheelSpeed(const double &_freq)
    {
      ...
      ROS_WARN("left_wheel_speed_magn 2 >> %f", this->left_wheel_speed_magn);
    }
    
    public: void SetRightWheelSpeed(const double &_magn)
    {
      this->right_wheel_speed_magn = _magn;
      ROS_WARN("right_wheel_speed_magn 2 >> %f", this->right_wheel_speed_magn);
    }
